...
The chart highlights the warm-cache, no-`node_modules` case. Aube uses
its global virtual store on this path, so projects mostly link to
package trees that already exist; pnpm v11 supports a global virtual
store too, but `enableGlobalVirtualStore` is not the default for
project installs.
